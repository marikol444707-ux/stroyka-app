...
Если нужен сильный SEO/AI-поиск, сделать отдельную статическую публичную страницу или SSR/prerender:

- `/` как быстрый статический лендинг с текстом, блоками, FAQ и формой заявки;
- ERP после входа остается React-приложением;
- приватные маршруты остаются закрытыми в `robots.txt`;
- sitemap расширяется публичными страницами: `/`, `/privacy`, `/terms`, `/features`, `/contacts`.
