...
- Проектные строки в `/project-catalog.html` ведут на deep-link главной страницы `/?project=CODE#projects`, чтобы пользователь, поисковик или AI-агент мог открыть конкретную интерактивную карточку.
- `/project-catalog.html` показывает в каждой проектной строке площадь, этажность/формат, краткую планировку и состав визуалов, поэтому статическая страница остается полезной без загрузки React.
- `/project-catalog.json` отдает тот же каталог в машинно-читаемом формате: направления, preview-картинки, проектные коды, статус `Проект-идея / пример для расчета`, площадь, этажность/формат, планировки, состав визуалов, полный `media[]` по каждому проекту и deep-link URL; `/project-catalog.html` дополнительно содержит JSON-LD `ItemList` на 45 проектов с изображениями и статусом проектного примера.
