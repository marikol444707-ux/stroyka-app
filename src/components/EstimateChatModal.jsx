...
export default function EstimateChatModal({
  ...
  const chatBusy = estimateChatHistoryLoading || estimateChatLoading;
      ...
      <div onClick={e=>e.stopPropagation()} className='mobile-modal estimate-chat-modal' style={{...card,padding:0,width:'640px',height:'700px',margin:isMobile?'12px':'20px',display:'flex',flexDirection:'column',overflow:'hidden'}}>
        <div style={{padding:'14px 18px',backgroundColor:'#0ea5e9',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
            <button disabled={chatBusy} onClick={clearChat} style={{background:'transparent',border:'1px solid rgba(255,255,255,0.4)',color:'white',cursor:chatBusy?'not-allowed':'pointer',opacity:chatBusy?0.6:1,padding:'4px 10px',borderRadius:'6px',fontSize:'11px'}}>🗑️ Очистить</button>
        ...
        <div style={{flex:1,overflowY:'auto',padding:'16px',display:'flex',flexDirection:'column',gap:'10px',backgroundColor:C.bg}}>
          {estimateChatMessages.length===0&&!chatBusy&&(
          ...
          {estimateChatHistoryLoading&&(<div style={{display:'flex',justifyContent:'flex-start'}}><div style={{padding:'12px 16px',borderRadius:'14px 14px 14px 4px',backgroundColor:darkMode?'#f8fafc':C.bgWhite,border:'1.5px solid '+(darkMode?'#cbd5e1':C.border),fontSize:'14px',color:darkMode?'#334155':C.textSec}}>⏳ Загружаю историю…</div></div>)}
        ...
        <div style={{padding:'12px 14px',borderTop:'1.5px solid '+C.border,backgroundColor:C.bgWhite,display:'flex',gap:'8px'}}>
          <input value={estimateChatInput} onChange={e=>setEstimateChatInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendEstimateChatMessage();}}} placeholder='Спросите по смете... (Enter — отправить)' disabled={chatBusy} style={{...inp,marginBottom:0,flex:1,fontSize:'13px'}}/>
          <button disabled={chatBusy||!estimateChatInput.trim()} onClick={sendEstimateChatMessage} style={{...btnO,padding:'10px 16px'}}>➤</button>
        </div>
      </div>
    </div>
  );
