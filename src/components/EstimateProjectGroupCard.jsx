...
export default function EstimateProjectGroupCard({
  ...
  return (
    <div className="estimate-project-card" style={{ ...card, marginBottom: '8px', overflow: 'hidden' }}>
      <div
        onClick={onToggle}
        role="button"
        aria-expanded={isOpen}
        style={{ padding: '10px 12px', backgroundColor: C.bg, borderBottom: isOpen ? '1px solid ' + C.border : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', flexWrap: 'wrap', cursor: 'pointer' }}
      >
      ...
      {isOpen && <div style={{ padding: '6px 8px' }}>
        {projectGroups.map(group => (
          <div key={group.key} style={{ marginBottom: '6px', border: '1px solid ' + C.border, borderRadius: '8px', overflow: 'hidden', backgroundColor: C.bgWhite }}>
            <div style={{ padding: '7px 10px', backgroundColor: C.bg, borderBottom: '1px solid ' + C.border, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
