...
export default function AppHeaderBar({
  ...
  const compact = Boolean(isCompactHeader || isMobile);
  ...
  return (
    <div style={{backgroundColor:C.bgWhite,padding:compact?'10px 12px':'12px 24px',borderBottom:'1.5px solid '+C.border,display:activePage==='dashboard'?'none':'flex',alignItems:compact?'flex-start':'center',gap:compact?'8px':'15px',flexWrap:'wrap',flexShrink:0,boxShadow:'0 1px 3px rgba(0,0,0,0.04)',width:'100%',maxWidth:'100vw',minWidth:0,boxSizing:'border-box',overflowX:'hidden',justifyContent:compact?'flex-start':'space-between'}}>
      ...
      <h2 style={{margin:0,color:C.text,fontSize:isMobile?'17px':'18px',lineHeight:1.2,fontWeight:'700',flex:compact?'1 1 calc(100% - 56px)':'1 1 auto',minWidth:0,alignSelf:'center',whiteSpace:isMobile?'normal':'nowrap',overflow:'hidden',textOverflow:'ellipsis',overflowWrap:'anywhere',maxWidth:isMobile?'calc(100vw - 92px)':'none'}}>{allMenuItems.find(m=>m.id===activePage)?.label||'СтройКа'}</h2>
      <div style={{flex:compact?'1 1 100%':'0 1 320px',maxWidth:compact?'none':'320px',position:'relative',order:compact?3:0,display:isMobile?'none':'block',minWidth:0}}>
      ...
      <div style={{position:'relative',display:'flex',gap:'8px',alignItems:'center',justifyContent:isMobile?'flex-start':'flex-end',flexWrap:'wrap',marginLeft:isMobile?0:'auto',width:isMobile?'100%':'auto',maxWidth:'100%',minWidth:0,overflowX:'hidden'}}>
        ...
        <button onClick={()=>setShowQuickActions(true)} title='Быстрые действия' style={{...headerButtonStyle,background:'linear-gradient(135deg,#f97316,#ea580c)',border:'none',color:'white',fontWeight:'700',fontSize:isMobile?'0':'13px',gap:'4px'}}>⚡ <span style={{fontSize:'13px',display:isMobile?'none':'inline'}}>Быстро</span></button>
        {['директор','зам_директора','system_owner'].includes(user.role)&&<button onClick={openSystemStatus} title='Статус системы' style={{...headerButtonStyle,gap:'5px',fontSize:isMobile?'0':'12px',fontWeight:'700'}}><Settings size={17}/><span style={{display:isMobile?'none':'inline'}}>Статус</span></button>}
