...
export default function AppHeaderBar({
  ...
  return (
    <div style={{backgroundColor:C.bgWhite,paddingTop:phoneLike?'calc(env(safe-area-inset-top, 0px) + 10px)':compact?'10px':'12px',paddingRight:compact?'12px':'24px',paddingBottom:compact?'10px':'12px',paddingLeft:compact?'12px':'24px',borderBottom:'1.5px solid '+C.border,display:activePage==='dashboard'?'none':'flex',alignItems:compact?'flex-start':'center',gap:compact?'8px':'15px',flexWrap:'wrap',flexShrink:0,boxShadow:'0 1px 3px rgba(0,0,0,0.04)',width:'100%',maxWidth:'100vw',minWidth:0,boxSizing:'border-box',overflowX:'hidden',justifyContent:compact?'flex-start':'space-between'}}>
      ...
      <h2 style={{margin:0,color:C.text,fontSize:phoneLike?'17px':'18px',lineHeight:1.2,fontWeight:'700',flex:compact?'1 1 calc(100% - 56px)':'1 1 auto',minWidth:0,alignSelf:'center',whiteSpace:phoneLike?'normal':'nowrap',overflow:'hidden',textOverflow:'ellipsis',overflowWrap:'anywhere',maxWidth:phoneLike?'calc(100vw - 92px)':'none'}}>{menuList.find(m=>m.id===activePage)?.label||'СтройКа'}</h2>
      <div style={{flex:compact?'1 1 100%':'0 1 320px',maxWidth:compact?'none':'320px',position:'relative',order:compact?3:0,display:phoneLike?'none':'block',minWidth:0}}>
        ...
        {showSearch&&resultList.length>0&&(<div style={{position:'absolute',top:'100%',left:0,right:0,backgroundColor:C.bgWhite,border:'1.5px solid '+C.border,borderRadius:'10px',boxShadow:'0 8px 25px rgba(0,0,0,0.1)',zIndex:1000,maxHeight:'280px',overflowY:'auto',marginTop:'4px'}}>{resultList.map((r,i)=>(<div key={i} onClick={()=>{openPage(r.page);changeGlobalSearch('');toggleSearch(false);}} style={{padding:'10px 15px',cursor:'pointer',borderBottom:'1px solid '+C.border,display:'flex',gap:'10px',alignItems:'center'}}><span style={{fontSize:'16px'}}>{r.icon}</span><div><b style={{fontSize:'13px',color:C.text}}>{r.title}</b><p style={{color:C.textSec,margin:0,fontSize:'11px'}}>{r.subtitle}</p></div></div>))}</div>)}
      </div>
      <div style={{position:'relative',display:'flex',gap:'8px',alignItems:'center',justifyContent:phoneLike?'flex-start':'flex-end',flexWrap:'wrap',marginLeft:phoneLike?0:'auto',width:phoneLike?'100%':'auto',maxWidth:'100%',minWidth:0,overflowX:'hidden'}}>
        ...
        <button onClick={()=>toggleChat(s=>!s)} title='Чат' style={{...headerButtonStyle,position:'relative'}}><MessageSquare size={18} color={C.textSec}/>{unreadMessagesCount>0&&<span style={{position:'absolute',top:'-4px',right:'-4px',backgroundColor:'#ef4444',color:'white',borderRadius:'50%',padding:'1px 5px',fontSize:'10px',fontWeight:'700',minWidth:'16px',textAlign:'center'}}>{unreadMessagesCount>99?'99+':unreadMessagesCount}</span>}</button>
        <NotificationsDropdown showNotifications={showNotifications} toggleNotifications={toggleNotifications} unreadNotifications={unreadNotifications} C={C} btnG={btnG} btnO={btnO} myNotifications={myNotifications} notifications={notifications} markMyNotificationsRead={markMyNotificationsRead} closeNotifications={closeNotifications} navigateTo={openPage} getNotifPage={getNotifPage} setShowNotifications={setShowNotifications} setNotifications={setNotifications} user={currentUser} setUser={setUser} API={API} isMobile={phoneLike} showVkConnect/>
      </div>
    </div>
  );
}
