...
export default function DashboardTopBar({
  ...
  const iconButtonStyle = {
    padding: phoneLike ? '7px 9px' : '8px 10px',
    ...
    color: '#94a3b8',
    flex: '0 0 auto',
    minWidth: phoneLike ? '36px' : undefined,
    minHeight: phoneLike ? '36px' : undefined,
  };

  return (
    <div style={{display:'flex',justifyContent:'space-between',alignItems:phoneLike?'flex-start':'center',marginBottom:phoneLike?'12px':'28px',flexWrap:'wrap',gap:phoneLike?'8px':'12px',width:'100%',minWidth:0,boxSizing:'border-box',overflow:phoneLike?'hidden':'visible'}}>
      ...
      <div style={{display:'flex',gap:phoneLike?'6px':'10px',alignItems:'center',justifyContent:phoneLike?'flex-start':'flex-end',flexWrap:'wrap',width:phoneLike?'100%':'auto',minWidth:0}}>
        ...
        <NotificationsDropdown showNotifications={showNotifications} toggleNotifications={toggleNotifications} unreadNotifications={unreadNotifications} C={theme} btnG={btnG} btnO={btnO} myNotifications={myNotifications} notifications={notifications} markMyNotificationsRead={markMyNotificationsRead} closeNotifications={closeNotifications} navigateTo={navigateTo} getNotifPage={getNotifPage} setShowNotifications={setShowNotifications} setNotifications={setNotifications} user={user} setUser={setUser} API={API} isMobile={phoneLike} buttonStyle={iconButtonStyle}/>
        <button onClick={openQuickActions} style={{background:'linear-gradient(135deg,#f97316,#ea580c)',border:'none',borderRadius:'12px',padding:phoneLike?'8px 12px':'10px 18px',minHeight:phoneLike?'36px':undefined,color:'white',fontWeight:'700',cursor:'pointer',fontSize:phoneLike?'12px':'13px',boxShadow:'0 8px 24px rgba(234,88,12,.35)',flex:'0 0 auto'}}>⚡ Быстро</button>
      </div>
    </div>
  );
}
