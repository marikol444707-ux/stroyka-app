...
export default function DashboardRisksPanel({
  ...
  const openReimburse = typeof setShowReimburseModal === 'function' ? setShowReimburseModal : () => {};
  const goToPage = typeof setActivePage === 'function' ? setActivePage : () => {};
  const changeWarehouseTab = typeof setWarehouseTab === 'function' ? setWarehouseTab : () => {};
  ...
  return (
    <div style={{background:'rgba(17,24,39,.88)',border:'1px solid rgba(148,163,184,.18)',borderRadius:'22px',padding:'20px',backdropFilter:'blur(24px)'}}>
      <h2 style={{margin:'0 0 12px',fontSize:'17px',color:'#f8fafc',display:'flex',alignItems:'center',gap:'8px'}}>⚠️ Предупреждения системы {safeRisks.length>0&&<span style={{fontSize:'12px',padding:'2px 8px',borderRadius:'10px',background:'rgba(239,68,68,.2)',color:'#fca5a5',fontWeight:'700'}}>{safeRisks.length}</span>}</h2>
      {safeRisks.length>0?safeRisks.map((r,i)=>{
        const cdng=r.severity==='danger';
        const isClickable = r.page || r.action;
        const handleClick = () => {
          if (r.action==='reimburse') { openReimburse(true); return; }
          if (r.page) { goToPage(r.page); if (r.tab) changeWarehouseTab(r.tab); }
        };
