...
export default function AppSidebar({
  ...
  const colorForRole = typeof roleColor === 'function' ? roleColor : () => C.accent || '#f97316';
  ...
  return (
    <>
      ...
      <div ref={sidebarRef} onMouseLeave={isMobile ? ()=>setSidebarOpen(false) : undefined} style={{position:'fixed',top:0,left:0,height:'100vh',width:isMobile?'min(86vw,300px)':'240px',backgroundColor:C.sidebar,color:'white',zIndex:300,transform:isOpen?'translateX(0)':'translateX(-100%)',transition:'transform 0.25s cubic-bezier(0.4,0,0.2,1)',display:'flex',flexDirection:'column',boxShadow:isOpen?'4px 0 30px rgba(0,0,0,0.15)':'none'}}>
        ...
        <div style={{padding:'14px',borderBottom:'1px solid rgba(255,255,255,0.08)'}}><div style={{backgroundColor:'rgba(255,255,255,0.06)',borderRadius:'10px',padding:'12px',display:'flex',alignItems:'center',gap:'10px'}}><div style={{width:'36px',height:'36px',borderRadius:'10px',backgroundColor:colorForRole(currentRole),display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px',color:'white',fontWeight:'800',flexShrink:0}}>{currentName.charAt(0)}</div><div style={{overflow:'hidden'}}><div style={{fontSize:'13px',fontWeight:'600',color:'white',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{currentName}</div><div style={{fontSize:'11px',color:'rgba(255,255,255,0.4)',marginTop:'1px'}}>{roleLabels[currentRole]||currentRole}</div></div></div></div>
