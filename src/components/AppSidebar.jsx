...
export default function AppSidebar({
  ...
  return (
    <>
      {isMobile && <div style={{position:'fixed',top:0,left:0,width:'16px',height:'100vh',zIndex:200}} onMouseEnter={()=>setSidebarVisible(true)}/>}
      <div ref={sidebarRef} onMouseLeave={isMobile ? ()=>setSidebarVisible(false) : undefined} style={{position:'fixed',top:0,left:0,height:'100vh',width:isMobile?'min(86vw,300px)':'240px',backgroundColor:C.sidebar,color:'white',zIndex:300,transform:isOpen?'translateX(0)':'translateX(-100%)',transition:'transform 0.25s cubic-bezier(0.4,0,0.2,1)',display:'flex',flexDirection:'column',boxShadow:isOpen?'4px 0 30px rgba(0,0,0,0.15)':'none'}}>
