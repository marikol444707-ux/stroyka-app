        ...
        <div style={{...dropdownStyle,backgroundColor:C.bgWhite,border:'1.5px solid '+C.border,borderRadius:isMobile?'16px':'14px',boxShadow:'0 8px 40px rgba(0,0,0,0.18)',zIndex:3000,overflowY:'auto',overflowX:'hidden'}}>
          <div style={{padding:isMobile?'12px':'14px 18px',borderBottom:'1.5px solid '+C.border,display:'flex',justifyContent:'space-between',alignItems:'center',gap:'8px',flexWrap:isMobile?'wrap':'nowrap'}}>
            ...
            <div style={{display:'flex',gap:'6px',alignItems:'center',marginLeft:'auto',flexShrink:0}}>
              <button onClick={markMyNotificationsRead} style={{...btnG,fontSize:isMobile?'10px':'11px',padding:isMobile?'5px 9px':'3px 10px',whiteSpace:'nowrap'}}>{markAllText}</button>
              <button onClick={closeNotifications} title="Свернуть" style={{...btnG,fontSize:'11px',padding:isMobile?'5px 8px':'3px 8px',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={13}/></button>
            </div>
          </div>
          ...
          {list.map(n=>(
            <div key={n.id} onClick={()=>markReadAndOpen(n)} style={{padding:isMobile?'12px':'12px 18px',borderBottom:'1px solid '+C.border,backgroundColor:n.read?'transparent':C.accentLight,cursor:'pointer'}}>
              <p style={{margin:0,fontSize:isMobile?'12px':'13px',lineHeight:1.35,color:C.text,overflowWrap:'anywhere',wordBreak:'break-word'}}>{n.text}</p>
