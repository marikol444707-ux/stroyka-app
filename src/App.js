...
function App() {
  ...
  const projectMeasurementBasisTotals = (projectName) => {
    ...
    return {
      ...
      opening_reveals: projectRooms.reduce((s,r)=>s+getRoomWindowRevealsTotal(r)+getRoomDoorRevealsTotal(r),0),
  ...
  const createSupplyRequestFromNormCoverage = async (row) => {
    ...
    const qtyRaw = window.prompt('Количество к заявке:', String(defaultQty || ''));
    if (qtyRaw === null) return;
    const qty = toNum(qtyRaw);
    if (qty <= 0) { alert('Количество должно быть больше 0'); return; }
    const unit = row.requiredUnit || row.rule.materialUnit || row.materialUnit || 'шт';
        ...
        <div style={{flex:1,overflowY:'auto',backgroundColor:activePage==='dashboard'?'#0b1120':C.bg,padding:activePage==='dashboard'?'0':'24px'}}>
          ...
          {activePage==='estimates'&&(<div>
            ...
            {estimatesTab==='norms'&&(<div>
              ...
              />
              {(()=>{const selectedProject=materialNormCoverageProject||visibleActiveProjects(projects||[])[0]?.name||'';const overrides=(materialNormOverrides||[]).filter(o=>!selectedProject||o.projectName===selectedProject);if(!overrides.length)return null;return(<div style={{...card,padding:'14px',marginBottom:'16px',backgroundColor:C.infoLight,border:'1.5px solid '+C.infoBorder}}>
                ...
                <div style={{display:'grid',gap:'8px'}}>
                  {overrides.slice(0,10).map(o=><div key={o.id} style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'minmax(220px,1.2fr) minmax(240px,1.5fr) 160px',gap:'10px',alignItems:'center',padding:'10px 12px',borderRadius:'8px',backgroundColor:C.bgWhite,border:'1px solid '+C.border}}>
