...
// Цвета берутся из CSS-переменных в App.css. Переключение темы — через document.documentElement.dataset.theme = 'light'|'dark'
const C = {
  bg:'var(--c-bg)', bgWhite:'var(--c-bg-white)', bgGray:'var(--c-bg-gray)',
  border:'var(--c-border)', borderDark:'var(--c-border-dark)',
  text:'var(--c-text)', textSec:'var(--c-text-sec)', textMuted:'var(--c-text-muted)',
  accent:'var(--c-accent)', accentDark:'var(--c-accent-dark)', accentLight:'var(--c-accent-light)', accentBorder:'var(--c-accent-border)',
  success:'var(--c-success)', successLight:'var(--c-success-light)', successBorder:'var(--c-success-border)',
  danger:'var(--c-danger)', dangerLight:'var(--c-danger-light)', dangerBorder:'var(--c-danger-border)',
  warning:'var(--c-warning)', warningLight:'var(--c-warning-light)', warningBorder:'var(--c-warning-border)',
  info:'var(--c-info)', infoLight:'var(--c-info-light)', infoBorder:'var(--c-info-border)',
  purple:'var(--c-purple)', purpleLight:'var(--c-purple-light)',
  sidebar:'var(--c-sidebar)', sidebarHover:'var(--c-sidebar-hover)',
};
...
function App() {
  ...
  const buildSpecJournalContent = (projectName, kind) => {
    ...
    const KW={welding:['свар','наплавка','шов','электрод'],concrete:['бетон','стяжк','залив','раствор','цемент'],assembly:['монтаж','сборн','установк','панел','плит','блок'],anticorrosion:['антикорр','покрас','грунт','окрашивание','лак'],insulation:['изоляц','утепл','гидро','пароизол','шумоизол'],piling:['свай','забивк','буронабивн','шпунт']};
    const map={welding:'СВАРОЧНЫХ',concrete:'БЕТОННЫХ',assembly:'МОНТАЖА СБОРНЫХ ж/б КОНСТРУКЦИЙ',anticorrosion:'АНТИКОРРОЗИОННОЙ ЗАЩИТЫ',insulation:'ИЗОЛЯЦИОННЫХ',piling:'СВАЙНЫХ'};
    const norm={welding:'РД-03-606-03',concrete:'ГОСТ 31108-2020 / СП 70.13330',assembly:'СП 70.13330',anticorrosion:'СНиП 3.04.03-85',insulation:'СП 71.13330',piling:'СП 24.13330'};
    const kws=KW[kind]||[];
    const filtered=works.filter(w=>{const d=(w.description||'').toLowerCase();return kws.some(k=>d.includes(k));});
    ...
    html+='<h2 style="text-align:center;margin:6px 0">ЖУРНАЛ '+map[kind]+' РАБОТ</h2>';
    html+='<p style="text-align:center;font-size:11px;color:#444">по '+norm[kind]+'</p>';
    html+='<p style="font-size:12px"><b>Объект:</b> '+projectName+' · <b>Подрядчик:</b> '+orgName+'</p>';
    if (filtered.length===0) html+='<p style="text-align:center;color:#888;font-size:11px;padding:20px">Записей по '+map[kind].toLowerCase()+' работам в журнале не найдено. Используются ключевые слова: '+kws.join(', ')+'</p>';
    else {
      ...
      filtered.forEach((w,i)=>{html+='<tr><td>'+(i+1)+'</td><td>'+(w.date||'')+'</td><td>'+(w.description||'')+(w.hiddenWork?' 🔒':'')+'</td><td>'+(w.unit||'')+'</td><td>'+(w.quantity||0)+'</td><td>'+(w.masterName||'')+'</td><td>'+(w.responsibleItr||'—')+'</td><td>'+(w.normatives||'—')+'</td><td>'+(w.qualityStatus||w.status||'')+'</td></tr>';});
      html+='</table>';
    }
    ...
    html+='</div>';
    html+='<p style="margin-top:16px;font-size:10px;color:#666;text-align:center">Журнал формируется автоматически из общего журнала работ по ключевым словам. По '+norm[kind]+' допустимо ведение отдельного спецжурнала.</p>';
    return html;
  };
                ...
                {brigadeContracts.filter(bc=>bc.projectName===(projects.find(p=>p.id===Number(masterProjectId))?.name)).map(bc=>(<div key={bc.id} style={{...card,padding:'12px',marginBottom:'8px',cursor:'pointer',border:'1.5px solid '+C.border}} onClick={async()=>{const res=await fetch(API+'/brigade-contract-items/'+bc.id);const items=await res.json();setSelectedBrigadeContract(bc);setBrigadeContractItems(items);if(bc.pricelistId) await loadPricelistItems(bc.pricelistId);}}>
                  ...
                  {selectedBrigadeContract?.id===bc.id&&brigadeContractItems.length>0&&(<div style={{marginTop:'10px',borderTop:'1px solid '+C.border,paddingTop:'10px'}}>
                    {brigadeContractItems.map((item,idx)=>(<div key={item.id} style={{display:'flex',alignItems:'center',gap:'8px',padding:'6px 0',borderBottom:'1px solid '+C.border}}>
                      ...
                      <input type='number' step='any' inputMode='decimal' placeholder={normalizeMeasure(1,item.unit).unit} value={item.doneQuantity?String(toNum(item.doneQuantity)*normalizeMeasure(1,item.unit).factor):''} onChange={async e=>{const f=normalizeMeasure(1,item.unit).factor;const userInput=toNum(e.target.value);const raw=userInput/f;const plan=Number(item.quantity)||0;if(plan>0&&raw>plan){alert('По смете план '+fmtMeasure(plan,item.unit)+'. Нельзя поставить больше — это перебор по смете.');return;}const val=plan>0?Math.min(raw,plan):raw;const updated={...item,doneQuantity:val,status:plan>0&&val>=plan?'Выполнено':val>0?'В работе':'Не начато'};setBrigadeContractItems(prev=>prev.map((it,i)=>i===idx?updated:it));await fetch(API+'/brigade-contract-items/'+item.id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(updated)});}} style={{...inp,marginBottom:0,width:'80px',fontSize:'12px',padding:'4px 6px'}}/>
                    </div>))}
          ...
          <div onClick={e=>e.stopPropagation()} style={{...card,padding:0,width:'min(900px,100%)',maxHeight:'92vh',display:'flex',flexDirection:'column',overflow:'hidden'}}>
            ...
            <div style={{flex:1,overflowY:'auto',padding:'18px 20px'}}>
              ...
              <div style={{padding:'12px',backgroundColor:C.bg,borderRadius:'10px',border:'1.5px solid '+C.border,marginBottom:'14px'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
                  <div><label style={labelStyle}>R изоляции ДО прокладки, МΩ</label><input type="number" step="any" inputMode="decimal" step="0.1" value={cb.insulationBefore||''} onChange={e=>upd('insulationBefore',e.target.value)} placeholder="напр. 500" style={inp}/></div>
                  <div><label style={labelStyle}>R изоляции ПОСЛЕ прокладки, МΩ</label><input type="number" step="any" inputMode="decimal" step="0.1" value={cb.insulationAfter||''} onChange={e=>upd('insulationAfter',e.target.value)} placeholder="напр. 480" style={inp}/></div>
                </div>
              ...
              <div style={{padding:'12px',backgroundColor:C.bg,borderRadius:'10px',border:'1.5px solid '+C.border,marginBottom:'14px'}}>
                ...
                <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:'10px'}}>
                  ...
                  <div><label style={labelStyle}>Длина проложенная, м</label><input type="number" step="any" inputMode="decimal" step="0.1" value={cb.lengthInstalled||''} onChange={e=>upd('lengthInstalled',e.target.value)} style={inp}/></div>
                  ...
                  <div style={{padding:'20px'}}>
                    ...
                    {activeProjectTab==='Непредвиденные'&&(<div>
                      ...
                      {showForm==='unexpected'&&(<div style={{...card,padding:'16px',marginBottom:'16px',backgroundColor:C.bg}}>
                        ...
                        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px'}}>
                          <input placeholder="Кол-во" type="number" step="any" inputMode="decimal" value={newUnexpected.quantity} onChange={e=>setNewUnexpected({...newUnexpected,quantity:e.target.value})} style={{...inp,marginBottom:0}}/>
...
<b style={{color:C.text,fontSize:'13px',display:'block',marginBottom:'10px'}}>Добавить работу вручную</b><div style={{display:'grid',gridTemplateColumns:'3fr 1fr 1fr 1fr 1fr auto',gap:'6px',alignItems:'center'}}><input placeholder='Наименование *' value={newBrigadeItem.name} onChange={e=>setNewBrigadeItem({...newBrigadeItem,name:e.target.value})} style={{...inp,marginBottom:0,fontSize:'12px'}}/><select value={newBrigadeItem.unit} onChange={e=>setNewBrigadeItem({...newBrigadeItem,unit:e.target.value})} style={{...inp,marginBottom:0,fontSize:'12px'}}>{UNITS.map(u=><option key={u}>{u}</option>)}</select><input placeholder='Объём' type='number' step='any' inputMode='decimal' value={newBrigadeItem.quantity} onChange={e=>setNewBrigadeItem({...newBrigadeItem,quantity:e.target.value})} style={{...inp,marginBottom:0,fontSize:'12px'}}/><input placeholder='Цена смета' type='number' step='any' inputMode='decimal' value={newBrigadeItem.priceSmeta} onChange={e=>setNewBrigadeItem({...newBrigadeItem,priceSmeta:e.target.value})} style={{...inp,marginBottom:0,fontSize:'12px'}}/><input placeholder='Цена бригаде' type='number' step='any' inputMode='decimal' value={newBrigadeItem.priceBrigade} onChange={e=>setNewBrigadeItem({...newBrigadeItem,priceBrigade:e.target.value})} style={{...inp,marginBottom:0,fontSize:'12px'}}/><button onClick={async()=>{if(!newBrigadeItem.name||!selectedBrigadeContract) return;const item={...newBrigadeItem,contractId:selectedBrigadeContract.id,doneQuantity:0,status:'Не начато'};const res=await fetch(API+'/brigade-contract-items',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(item)});const saved=await res.json();setBrigadeContractItems(prev=>[...prev,{...item,id:saved.id}]);setNewBrigadeItem({name:'',unit:'м',quantity:'',priceSmeta:'',priceBrigade:'',estimateSection:''});}} style={{...btnO,padding:'7px 12px'}}><Plus size={13}/></button></div></div><div style={{...card,padding:'12px 16px',marginBottom:'8px',backgroundColor:C.bg,display:isLeadership()?'flex':'none',gap:'10px',alignItems:'center',flexWrap:'wrap'}}>
              ...
              <input type='number' step='any' inputMode='decimal' value={brigadeCoef} onChange={e=>setBrigadeCoef(e.target.value)} style={{...inp,marginBottom:0,width:'80px',fontSize:'12px',padding:'4px 8px'}} step='0.05' min='0.1' max='1'/>
            ...
            {warehouseTab==='invoices'&&(<div>
              ...
              {showForm&&(<div style={{...card,padding:'20px',marginBottom:'20px'}}>
                ...
                {newInvoice.items.map((item,idx)=>(<div key={idx} style={{display:'grid',gridTemplateColumns:'3fr 1fr 1fr 1fr 2fr auto',gap:'6px',marginBottom:'8px',alignItems:'center'}}>
                  ...
                  <input placeholder="Кол-во *" type="number" step="any" inputMode="decimal" value={item.quantity} onChange={e=>{const items=[...newInvoice.items];items[idx]={...items[idx],quantity:e.target.value};setNewInvoice({...newInvoice,items});}} style={{...inp,marginBottom:0,fontSize:'12px'}}/>
