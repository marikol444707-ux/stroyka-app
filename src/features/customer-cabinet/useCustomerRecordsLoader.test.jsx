...
test('late response from previous company cannot overwrite the new scope', async () => {
  ...
  rerender({ company: 3 });
  await act(async () => { for (const resolve of pending.slice(5)) resolve({ ok: true, json: async () => [] }); });
  await act(async () => { for (const resolve of pending.slice(0, 5)) resolve({ ok: true, json: async () => [{ companyId: 2, projectId: 3, side: 'customer' }] }); });
  expect(result.current.docs).toEqual([]);
