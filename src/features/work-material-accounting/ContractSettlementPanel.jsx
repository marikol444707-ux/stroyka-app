  ...
  return <div className="work-ledger" style={{ '--ledger-bg': C.card || C.bg, '--ledger-text': C.text, '--ledger-border': C.border }}>
    <section><h3>Акты и оплата по договору</h3><p>{contract.brigadeName} · {contract.projectName} · договор №{contract.id}</p>
      ...
      {data && <>
        ...
        {data.fineAllocations.map(fine => <p key={`${fine.source || 'material'}:${fine.incidentId || fine.defectId}`}>{fine.source === 'tool' ? `Инструмент: ${fine.toolName} · происшествие №${fine.incidentId}` : `Брак №${fine.defectId} · ЖПР №${fine.journalId}`}: <b>{formatMoney(fine.amount)}</b><br />{fine.reason} · {fine.contractEvidence}</p>)}
