...
export function buildSettlementAct(act) {
  ...
  return `<h2>АКТ ВЫПОЛНЕННЫХ РАБОТ №${escape(act.id)}</h2>
    ...
    <h3>Штрафы</h3>${s.fines.length ? `<table border="1" cellpadding="6" style="width:100%;border-collapse:collapse"><tr><th>Основание</th><th>Материалы и стоимость</th><th>Включено в этот акт, ₽</th></tr>
    ${s.fines.map(f => `<tr><td>Брак №${escape(f.defectId)}, ЖПР №${escape(f.journalId)}<br>Решение №${escape(f.decisionId)}<br>${escape(f.reason)}<br>${escape(f.contractEvidence)}</td><td>${(f.valuations || []).map(v => `${escape(v.name || 'Материал №' + v.entryId)}: ${escape(v.quantity)} ${escape(v.unit || '')} × ${money(v.unitPrice)} ₽; ${escape(v.priceEvidence)}`).join('<br>')}</td><td>${money(f.amount)}</td></tr>`).join('')}</table>` : '<p>Подтверждённых штрафов нет.</p>'}
