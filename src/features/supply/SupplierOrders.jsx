...
export function supplierDocumentUrl(value,fileSrc) {
...
export default function SupplierOrders({API,user,C,onOpen,fileSrc}) {
 ...
 return <section className="supplier-orders" aria-label="Подтверждённые заказы" style={{color:C.text}}>
  ...
  {inbox.status==='ready' && <SupplierOrderCards orders={orders} onOpen={onOpen} fileSrc={fileSrc}/>}
 </section>;
}

export function SupplierOrderCards({orders,onOpen,fileSrc}) {
 return <>
  {orders.map(order=><article className="supplier-order" key={order.id}>
