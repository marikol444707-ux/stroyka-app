...
export default function useSupplierTeam(API,user) {
 ...
 const reload=useCallback(async()=>{
  ...
  setState(previous => previous.scope===scope && previous.status==='ready'
   ? {...previous,refreshing:true} : {scope,status:'loading',suppliers:[]});
  try {
 ...
 const updateProfile=useCallback(profile=>{
  ...
  setState(previous=>previous.scope===scope && previous.status==='ready'
   ? {...previous,suppliers:previous.suppliers.map(supplier=>String(supplier.id)===String(profile.supplierId)
      ? {...supplier,name:profile.fields.name} : supplier)} : previous);
 },[scope]);
 return {...(state.scope===scope?state:{status:'loading',suppliers:[]}),reload,updateProfile};
}
