...
import { attentionLabels, responseDeadlineLabel } from './supplierDeadlines';

const PAGE_SIZE=20;
export default function SupplierRequestRegistry({requests=[], offers=[], onOpen, selectedId, C, busy=false, invoices=[], deliveries=[]}) {
  const [filters,setFilters]=useState({query:'',company:'',status:'',attention:''});
  const [page,setPage]=useState(1);
  const [now,setNow]=useState(Date.now);
  React.useEffect(()=>{const timer=setInterval(()=>setNow(Date.now()),30000);return ()=>clearInterval(timer);},[]);
  const rows=useMemo(()=>registryRows(requests,offers,invoices,deliveries,now),[requests,offers,invoices,deliveries,now]);
  ...
  return <section className="supplier-registry" aria-label="Реестр заявок" style={{color:C.text,background:C.bgCard,borderColor:C.border}}>
    ...
    <div className="supplier-attention" aria-label="Внимание по заявкам">{Object.entries(attentionLabels).map(([key,label])=><button type="button" key={key} aria-pressed={filters.attention===key} onClick={()=>change('attention',filters.attention===key?'':key)}>{label}<strong>{rows.filter(row=>row.attention.includes(key)).length}</strong></button>)}</div>
    <div className="supplier-registry-filters">
      ...
      <button type="button" onClick={()=>{setFilters({query:'',company:'',status:'',attention:''});setPage(1);}}>Сбросить фильтры</button>
      <button type="button" disabled={!filtered.length || busy} onClick={()=>downloadRegistryCsv(filtered)}>Выгрузить CSV</button>
    </div>
    <p className="supplier-registry-count" role="status">Найдено: {filtered.length} из {rows.length}{filters.attention && ' · '+attentionLabels[filters.attention]}</p>
    <table><thead><tr><th>Заявка / заказчик</th><th>Материалы / объект</th><th>Запрос КП получен</th><th>Состояние КП</th><th><span className="supplier-sr-only">Открыть</span></th></tr></thead>
      <tbody>{visible.map(row=><tr key={row.id} aria-selected={String(selectedId)===String(row.id)}>
        ...
        <td data-label="Запрос КП получен">{registryDate(row.requestedAt)}<div>Ответить до: {responseDeadlineLabel(row.responseDueAt)}</div></td>
