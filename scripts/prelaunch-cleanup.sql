...
\echo '--- Applying prelaunch cleanup ---'
BEGIN;
...
BEGIN
  ...
  DELETE FROM users WHERE lower(email) <> lower(keep_email);
  UPDATE users
  SET
    role = 'директор',
    active = TRUE,
...
\echo '--- Cleanup result ---'
SELECT id, name, email, role, active, assigned_projects FROM users ORDER BY id;
SELECT 'projects' AS table_name, count(*) FROM projects
UNION ALL SELECT 'staff', count(*) FROM staff
UNION ALL SELECT 'estimates', count(*) FROM estimates
UNION ALL SELECT 'work_journal', count(*) FROM work_journal
UNION ALL SELECT 'materials', count(*) FROM materials
UNION ALL SELECT 'warehouse_main', count(*) FROM warehouse_main
UNION ALL SELECT 'ai_tasks', count(*) FROM ai_tasks;
