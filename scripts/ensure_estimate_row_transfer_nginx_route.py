#!/usr/bin/env python3
"""Idempotently add E4.2 transfer-plan proxy routes to an active Nginx site."""
...
ROUTES = (
    (
        re.compile(r"(?m)^\s*location\s*=\s*/estimate-row-transfer-plans\s*\{"),
        (
...
def ensure_transfer_routes(text: str) -> tuple[str, list[str]]:
...
def update_file(
    ...
    updated, added = ensure_transfer_routes(original)
    if not added:
        return None, []
    ...
    backup = destination / f"{target.name}.before-estimate-row-transfer.{stamp}"
...
def main() -> None:
    ...
    if not added:
        print("Маршруты E4.2 уже настроены; изменений нет.")
        return
