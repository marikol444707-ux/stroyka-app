...
STEPS = [
    ...
    {
        "name": "role-package",
        "title": "Исполнитель видит только назначенный объект/пакет",
        "command": ["python3", "scripts/smoke-role-package-access.py"],
    },
    {
        "name": "staff-access",
        "title": "Карточка сотрудника -> системный доступ -> смена пароля",
        "command": ["python3", "scripts/smoke-staff-access.py"],
    },
    {
        "name": "role-matrix",
        "title": "Матрица входов и API по всем основным ролям",
        "command": ["python3", "scripts/smoke-role-matrix.py"],
    },
]
...
def preflight_login():
    ...
    if status != 200:
        raise SystemExit(
            "Preflight login не прошел, цепочки D/E не запускаю, чтобы не заблокировать аккаунт. "
...
if __name__ == "__main__":
    try:
        ...
        print("FAIL smoke:work-roles:", exc, file=sys.stderr)
        raise SystemExit(1)
