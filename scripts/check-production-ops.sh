...
note "OK uploads current: $uploads_path -> $(dir_stats "$uploads_path")"
if [[ "$LEGACY_UPLOADS_DIR" != "$UPLOADS_DIR" && -d "$legacy_uploads_path" ]]; then
  ...
  if [[ "${legacy_files:-0}" != "0" ]]; then
    warn "legacy uploads directory has files: $legacy_uploads_path ($(dir_stats "$legacy_uploads_path"))"
  fi
fi

if [[ -f "$APP_DIR/ops-nginx-stroyka-public-api.conf" ]]; then
  if rg -q "limit_req" "$APP_DIR/ops-nginx-stroyka-public-api.conf"; then
...
if [[ -f "$APP_DIR/docs/ddos-and-ai-indexing.md" ]]; then
  if rg -q "limit_req_zone" "$APP_DIR/docs/ddos-and-ai-indexing.md" && rg -q "client_max_body_size" "$APP_DIR/docs/ddos-and-ai-indexing.md"; then
