"""Guarded ownership migration for legacy audit_log rows."""
...
APPLY_CONFIRMATION = "APPLY_AUDIT_LOG_OWNERSHIP"
...
def _migration_result(evidence, status, reason, scope=None, company_id=None, project_id=None):
    return {
        ...
        "entityType": evidence.get("entityType") or "",
        "status": status,
        "reason": reason,
        "proposedScope": scope,
...
def classify_migration_row(evidence, stored_owner=None, allow_legacy=False):
    ...
    if has_stored_values:
        ...
        if stored_scope == "company" and stored_company_id:
            expected = (item.get("scope"), item.get("companyId"), item.get("projectId"))
            stored = (stored_scope, stored_company_id, stored_project_id)
    ...
    return _migration_result(item, item.get("status") or "unresolved", item.get("reason") or "unresolved")
...
def build_migration_report(columns, classified, review_hash):
    ...
    return {
        ...
        "summary": {
            ...
            "storedCompanyRows": sum(1 for item in stored if item.get("proposedScope") == "company"),
            "storedPlatformRows": sum(1 for item in stored if item.get("proposedScope") == "platform"),
            "storedLegacyRows": sum(1 for item in stored if item.get("proposedScope") == "legacy"),
...
def _stored_owners(cur, columns):
    if not OWNER_COLUMNS.issubset(columns):
        return {}
    cur.execute("SELECT id,owner_scope,company_id,project_id FROM audit_log ORDER BY id")
...
def collect_ownership(cur, legacy_review_sha=None):
    ...
    evidence = classify_ownership_rows(rows)
...
def _plan_sha256(classified):
    ...
    for item in classified or []:
        ...
        if not record_id or scope not in {"company", "platform", "legacy"}:
            raise ValueError("ready audit row has invalid owner identity")
        if scope == "company" and not company_id:
            raise ValueError("company audit row has no company")
        if scope in {"platform", "legacy"} and (company_id or project_id):
            raise ValueError("non-company audit row contains tenant IDs")
...
def _ensure_schema(cur):
    ...
    cur.execute("LOCK TABLE audit_log IN ACCESS EXCLUSIVE MODE")
    cur.execute("ALTER TABLE audit_log ADD COLUMN IF NOT EXISTS owner_scope TEXT")
    cur.execute("ALTER TABLE audit_log ADD COLUMN IF NOT EXISTS company_id INT")
    cur.execute("ALTER TABLE audit_log ADD COLUMN IF NOT EXISTS project_id INT")
    cur.execute(
    ...
    cur.execute(
        """DO $$ BEGIN
               IF NOT EXISTS (
                   SELECT 1 FROM pg_constraint WHERE conname='ck_audit_log_owner_scope'
               ) THEN
...
def _apply_ready_rows(cur, ready):
    ...
    cur.execute(
        """UPDATE audit_log a
            ...
            WHERE a.id=owners.id
              AND a.owner_scope IS NULL AND a.company_id IS NULL AND a.project_id IS NULL""",
        (
...
def run_migration(
    ...
    if not apply:
        ...
        try:
            ...
            result = _base_result(
                build_migration_report(columns, classified, review_hash), "dry-run", classified
            )
            conn.rollback()
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Guarded ownership migration for audit_log")
