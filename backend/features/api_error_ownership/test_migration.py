...
def evidence(record_id=1, status="verified", scope="company", company_id=4):
    return {
        "recordId": record_id,
        "scope": scope if status == "verified" else None,
        "companyId": company_id if status == "verified" else None,
        "status": status,
        "reason": "verified_unique_actor_company" if status == "verified" else "actor_missing",
    }
...
class FakeCursor:
    ...
    def execute(self, sql, params=()):
        ...
        if compact.startswith("UPDATE api_errors"):
...
class ApiErrorOwnershipMigrationTests(unittest.TestCase):
    def test_verified_company_and_platform_rows_are_ready(self):
        company = classify_migration_row(evidence())
        platform = classify_migration_row(evidence(2, scope="platform", company_id=None))
        ...
        self.assertEqual(
            (platform["status"], platform["proposedScope"]),
            ("ready", "platform"),
        )

    def test_unresolved_requires_exact_review_set_before_legacy(self):
        ...
        self.assertEqual((blocked["status"], blocked["reason"]), ("unresolved", "actor_missing"))
        self.assertEqual(
            (accepted["status"], accepted["proposedScope"], accepted["reason"]),
            ("ready", "legacy", "explicit_review_set_as_legacy"),
        )

    def test_ambiguous_or_mismatched_cannot_be_hidden_as_legacy(self):
    ...
    def test_stored_owner_mismatch_fails_closed(self):
        result = classify_migration_row(
            evidence(),
            {"owner_scope": "company", "company_id": 5, "project_id": None},
        )

        self.assertEqual(
            (result["status"], result["reason"]),
            ("mismatched", "stored_company_owner_mismatch"),
        )

    def test_report_is_strict_only_after_columns_and_rows_are_stored(self):
        ready = classify_migration_row(evidence())
        stored = classify_migration_row(
            evidence(),
            {"owner_scope": "company", "company_id": 4, "project_id": None},
        )

        before = build_migration_report(set(), [ready], "a" * 64)
        after = build_migration_report(
    ...
    def test_collect_rejects_changed_review_sha(self):
        rows = {
            "api_errors": [{"id": 9, "user_id": None, "method": "GET"}],
            "users": [],
            "user_company_roles": [],
        }
    ...
    def test_apply_adds_schema_and_commits_only_after_strict_postcheck(self):
        ...
        self.assertIn("LOCK TABLE api_errors IN ACCESS EXCLUSIVE MODE", sql)
        self.assertIn("ck_api_errors_owner_scope", sql)

    def test_apply_rejects_plan_drift_before_update(self):
        cursor = FakeCursor()
        connection = FakeConnection(cursor)
        audited = [classify_migration_row(evidence())]
        changed = [classify_migration_row(evidence(company_id=8))]
        with patch.object(
            migration_module,
            "collect_ownership",
        ...
        self.assertFalse(any(sql.startswith("UPDATE api_errors") for sql, _ in cursor.calls))
        self.assertTrue(connection.rolled_back)

    def test_apply_rolls_back_on_write_conflict(self):
