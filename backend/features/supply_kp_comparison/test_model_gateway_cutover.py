...
class SupplyKpComparisonGatewayCutoverTest(unittest.TestCase):
    def test_legacy_rollback_preserves_the_existing_sdk_request(self):
        ...
        class FakeResponses:
            def create(self, **values):
                ...
                return SimpleNamespace(output_text="  Выберите поставщика №1.  ")
        ...
        with patch.dict(sys.modules, {"openai": fake_openai}):
            answer = model.generate_supply_kp_comparison_legacy(
                "Полный промпт",
                "Инструкции",
                "private-key",
                "folder-1",
    ...
    def test_gateway_builds_the_equivalent_neutral_request(self):
        ...
        with patch.object(model, "build_yandex_model_adapter", adapter_factory):
            answer = model.generate_supply_kp_comparison_gateway(
                "Полный промпт",
                "Инструкции",
                "private-key",
                "folder-1",
    ...
    def test_gateway_failure_raises_a_fixed_non_leaking_error(self):
        ...
        with patch.object(
            ...
            with self.assertRaises(ModelGatewayError) as caught:
                model.generate_supply_kp_comparison_gateway(
                    "Полный промпт",
                    "Инструкции",
                    "private-key",
                    "folder-1",
    ...
    def test_cutover_defaults_to_legacy_and_enables_only_this_gateway(self):
        ...
        with (
            ...
            patch.object(
                model,
                "generate_supply_kp_comparison_gateway",
                lambda *_args: calls.append("gateway") or "new",
            ),
        ):
            old = model.generate_supply_kp_comparison(
                "prompt",
                "instructions",
                "key",
                "folder",
            )
            new = model.generate_supply_kp_comparison(
                "prompt",
                "instructions",
                "key",
                "folder",
    ...
    def test_direct_provider_access_is_confined_to_the_rollback_function(self):
        ...
        self.assertIn(
            "OpenAI",
            ast.unparse(functions["generate_supply_kp_comparison_legacy"]),
        )
        self.assertNotIn(
            "OpenAI",
            ast.unparse(functions["generate_supply_kp_comparison_gateway"]),
        )
        self.assertNotIn(
            "OpenAI",
            ast.unparse(functions["generate_supply_kp_comparison"]),
        )
    ...
    def test_route_delegates_only_the_text_transport_with_a_safe_cutover(self):
        ...
        self.assertIn("SUPPLY_KP_COMPARISON_MODEL_GATEWAY_ENABLED", route_source)
