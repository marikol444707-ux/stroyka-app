...
def register_estimate_row_transfer_module(app, deps):
    ...
    @app.post("/estimate-row-transfer-plans/{plan_id}/assignment-apply")
    def apply_transfer_plan_assignments(
        plan_id: int,
        data: dict,
        x_company_id: Optional[str] = Header(default=None, alias="X-Company-Id"),
        x_company_mode: Optional[str] = Header(default=None, alias="X-Company-Mode"),
        ...
        if not _positive_int(plan_id):
            raise HTTPException(status_code=422, detail="assignment_apply_payload_invalid")
        try:
            payload = normalize_assignment_apply_payload(data)
        except AssignmentApplyError as exc:
            raise HTTPException(status_code=422, detail=exc.code)
        ...
        try:
            ...
            if (
                ...
                abort(409, "assignment_plan_not_approved")
            if payload["planSha256"] != canonical.get("planSha256"):
                abort(409, "assignment_plan_hash_mismatch")
            try:
                result = apply_assignments(cur, stored=stored, actor=actor)
            except AssignmentApplyError as exc:
                abort(409, exc.code)
