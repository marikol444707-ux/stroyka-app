...
class EstimateRowTransferSchemaPlanTests(unittest.TestCase):
    def test_fresh_plan_is_deterministic_and_contains_only_schema_changes(self):
        ...
        self.assertIn("NOTIN('1','2')", "".join(sql.split()))
        self.assertIn("{estimateLineage,companyId}", sql)
        self.assertIn("{estimateLineage,projectId}", sql)
        self.assertIn("target_version.sections_json::jsonb #>> ARRAY[", sql)
        self.assertIn("'itemType'", sql)
        self.assertIn("'material','materials','материал','материалы'", sql)
