...
### 🔥 Ф1 — Реорганизация навигации + связки журнала [L]

- [ ] Переименовать раздел проекта «📋 Документы» → «📚 Журналы»
- [ ] Создать новый раздел «📋 Документы» с вкладками: КС-2 / КС-3 / Паспорт / АОСР / Акты технадзора / Предписания
- [ ] Перенести КС-2 / КС-3 / Паспорт из «💰 Учёт» в раздел «📋 Документы» проекта
- [ ] Объединить «Журнал работ» (из Документов) и «Журнал» (из Работ) в одну вкладку с переключателем вида: 📅 Дневной / 📋 Таблица КС-6а
- [ ] Связка журнала ↔ АОСР: значки 🔒 / ✅ / ⏳ в записях журнала; кнопка «Открыть АОСР» в модалке записи
- [ ] Главный экран «📚 Журналы» — карточки всех журналов со счётчиками («45 записей · посл. 23.05»)
- [ ] Непредвиденные работы: при утверждении автоматически создаётся запись в `work_journal` (с привязкой `unexpected_work_id`)
- [ ] Значок «🆕 ДОП» для непредвиденных работ в журнале
- [ ] Включить непредвиденные в КС-2 отдельным разделом
- [ ] Базовая система уведомлений между ролями (внутрисистемные плашки в кабинетах)

### 🔥 Ф2 — Журнал входного контроля материалов [M]

- [ ] Таблица `material_inspection_journal` по СП 48.13330.2019 §7.1
- [ ] Автозаполнение из накладных `warehouse_invoices` **БЕЗ сумм** (только количество, поставщик, дата)
- [ ] Endpoints `/material-inspection` (GET/POST/PUT) + UI вкладка в «📚 Журналы»
- [ ] Модалка записи (паспорт качества №, сертификат соответствия №, протокол испытаний №, результат осмотра)
- [ ] AI-подсказка применимых нормативов (ГОСТ/СП по типу материала)
- [ ] Печать журнала входного контроля по форме СП 48.13330
- [ ] М-15 — Накладная на отпуск материалов со склада (внутреннее перемещение)

### 🟡 Ф3 — Журнал кабельной продукции [M]

- [ ] Таблица `cable_journal` по СП 76.13330
- [ ] Автоопределение кабеля по марке (префиксы ВВГ / КВВГ / ПвВ / СИП / UTP / FTP и т.д.)
- [ ] Парсинг сечения жилы и количества жил из названия (ВВГнг 3х2.5 → 3 жилы × 2.5 мм²)
- [ ] Замер сопротивления изоляции **до** и **после** прокладки (МΩ)
- [ ] № барабана / бухты, способ прокладки, место (объект/этаж/помещение)
- [ ] Печать журнала кабельной продукции
- [ ] AI-подсказка минимального R изоляции по ПУЭ для марки кабеля

### 🔥 Ф4 — Кабинет технадзора (расширение) [M]

- [ ] Доступ к АОСР с подписью через **SMS-код** (простая цифровая подпись)
- [ ] Загрузка своих актов осмотра/обследования (новая таблица `supervisor_acts`)
- [ ] Фотофиксация при выдаче предписаний (сейчас не прикрепляются)
- [ ] Месячный отчёт по объекту (печатная форма с агрегацией всех замечаний и подписей)
- [ ] Доступ к журналу входного контроля материалов (только чтение)

### 🔥 Ф5a — Кабинет бухгалтера (расширение) [L]

- [ ] Разбить раздел «💰 Бухгалтерия» на вкладки: 📊 Сводка / 🏗 По объектам / 🧾 Договоры / 📄 Акты / 📦 Счета и накладные / 💸 Платежи / 🖨 Печатные формы / 👥 Расчёт зарплаты / 📊 Отчёты
- [ ] Связка АОСР → оплата: статусы «Подписан / К оплате / Оплачен» в актах
- [ ] Уведомление бухгалтеру при подписи АОСР: «🔔 АОСР № … подписан — можно платить бригаде»
- [ ] Кнопка «💰 Оплатить по этому АОСР» в её кабинете
- [ ] М-29 — Отчёт о расходе материалов (план по смете vs факт по `material_transfers`)
- [ ] Раздел «Непредвиденные к оплате» (только после подписания доп.соглашения заказчиком)
- [ ] Расчёт зарплаты помесячно по `staff.pay_type`
- [ ] **Авансовые отчёты** сотрудников (командировки, мелкий нал, бензин)
- [ ] **Командировочные** (если объекты в разных городах) — суточные, проживание, проезд
- [ ] **Книга покупок и продаж** (для НДС-отчётности на ОСН)
- [ ] **Расчёт себестоимости по объекту** — план (бюджет) vs факт (все траты по проекту), реальная маржа
- [ ] **Входящие счета от поставщиков на утверждение** (раздел «Утвердить к оплате»)
- [ ] Связка **Поставщик → Бухгалтер**: загруженный счёт от поставщика появляется у бухгалтера на утверждение
- [ ] Связка **Бухгалтер → Снабженец**: утверждённый счёт = «можно закупать», уведомление снабженцу

### 🔥 Ф5b — Кабинет заказчика (расширение) [L]

- [ ] 📐 **Смета** — раздел с возможностью утверждения и подписи (через SMS-код)
- [ ] 🔒 **АОСР на подпись** — список с цифровой подписью
- [ ] 📄 **КС-2 на согласование** — получение, замечания, подпись
- [ ] 📋 **КС-3** — справка о стоимости
- [ ] 📷 **Фото-отчёт по неделям** — хронология фотофиксации работ
- [ ] 📖 **Журнал работ** — только чтение
- [ ] ⚠️ **Свои замечания/предписания** — заказчик может выдавать
- [ ] 💬 **Чат с прорабом** — текущая коммуникация по объекту
- [ ] 💰 **История платежей** — что заплачено / остаток / следующий платёж
- [ ] **Принцип «всё из сметы заказчика»** — техническая защита: КС-2 строится **только** из позиций сметы с `priceWork+priceMaterial`, не из `workJournal` (запрет на свободные позиции)
- [ ] **PDF-экспорт** документов вместо HTML-печати (через библиотеку)
- [ ] **Согласование непредвиденных** работ → автоформирование доп.соглашения к договору с подписью

### 🟡 Ф6 — Унификация старых журналов [M]

- [ ] Журнал ТБ переделать по ГОСТ 12.0.004-2015 (вводный / первичный / повторный / внеплановый инструктаж)
- [ ] AI-помощь с текстом инструктажа под тип работ
- [ ] ЖПР переделать по РД-11-05-2007 (титульный лист + 7 разделов: ИТР, стройконтроль, выполнение работ, входной контроль, спецжурналы, замечания органов)
- [ ] Предписания по форме МДС 12-46.2008 (форма протокола/акта осмотра)
- [ ] Таблица `audit_log` — кто что и когда изменил (для разбирательств)
- [ ] Версионирование критических документов (АОСР, КС-2 — snapshot при каждом изменении, как уже сделано для смет)
- [ ] **Журнал авторского надзора** (если есть проектная организация надзирающая)
