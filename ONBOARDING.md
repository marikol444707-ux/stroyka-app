...
- [ ] Книга покупок и продаж (НДС-отчётность на ОСН) — отчёт-агрегация из supplier_invoices
- [ ] Полная автоинтеграция Поставщик→Бухгалтер (поставщик в портале загружает счёт → автосоздание supplier_invoice)
- [ ] Уведомление снабженцу при утверждении счёта (notifications, требует интеграции с computeNotifications)
