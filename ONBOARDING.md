...
### 🟡 Ф1.2 — Доделка реорганизации (что осталось от Ф1) [M]

- [ ] Объединить дневной журнал и таблицу КС-6а в одной вкладке через переключатель (сейчас табличный вид — через кнопку «📋 Таблица КС-6а» открывает модалку, надо встроить inline)
- [ ] Значки **статуса АОСР** ✅ / ⏳ в записях журнала рядом с 🔒 (сейчас только 🔒)
- [ ] Базовая система уведомлений между ролями (таблица `notifications` + плашки в кабинетах: «АОСР на подпись», «КС-2 готов», «непредвиденная на согласование»)
- [ ] Объединить две точки печатных форм по объекту: сейчас и в «📋 Документы» проекта, и в «💰 Бухгалтерия» — оставить одну

### 🟡 Ф2.1 — Что осталось от Ф2 [S]

- [ ] М-15 — Накладная на отпуск материалов со склада (печатная форма по существующим `material_transfers`)


### 🟡 Ф4.1 — Что осталось от Ф4 [S]

- [ ] Месячный отчёт технадзора по объекту (печать с агрегацией АОСР + актов осмотра + предписаний)
- [ ] SMS-код вместо «впиши ФИО руками» для подписи АОСР (см. секцию «Инфраструктура»)

### 🟡 Ф5a.1 — Что осталось от Ф5a [L]

- [ ] Уведомление бухгалтеру при подписи АОСР («🔔 АОСР подписан — можно платить»)
- [ ] М-29 — Отчёт о расходе материалов (план по смете vs факт по `material_transfers`)
- [ ] Раздел «Непредвиденные к оплате» (после подписания доп.соглашения заказчиком)
- [ ] Расчёт зарплаты помесячно по `staff.pay_type`
- [ ] Авансовые отчёты сотрудников (командировки, мелкий нал, бензин)
- [ ] Командировочные (суточные / проживание / проезд)
- [ ] Книга покупок и продаж (НДС-отчётность на ОСН)
- [ ] Расчёт себестоимости по объекту — план vs факт по проекту
- [ ] Входящие счета от поставщиков на утверждение
- [ ] Связка Поставщик → Бухгалтер (счёт на утверждение)
- [ ] Связка Бухгалтер → Снабженец (счёт утверждён → можно закупать)

### 🟡 Ф5b.1 — Что осталось от Ф5b [M]
...
- [ ] Автоформирование доп.соглашения к договору при согласовании непредвиденной работы (сейчас просто меняется статус на «Утверждено»)
- [ ] SMS-код для цифровой подписи (сейчас «впиши ФИО» вручную)

### 🟡 Ф6.1 — Что осталось от Ф6 [M]

- [ ] Журнал ТБ во фронте по ГОСТ 12.0.004-2015 (форма с типом инструктажа + кнопка «🤖 Сгенерировать текст» — backend готов: POST /tb-journal/ai-generate)
- [ ] Версионирование критических документов (АОСР, КС-2 — snapshot при каждом изменении)
- [ ] Журнал авторского надзора (если есть проектная организация)
- [ ] Журнал замечаний контролирующих органов (ГСН / ГПН / Роспотребнадзор)
- [ ] UI для просмотра audit_log (записи кто что когда — backend готов: GET /audit-log)
- [ ] Интеграция audit_log с критичными endpoints (PUT/DELETE → автоматическая запись)

### 🔥 Ф7 — Исполнительная документация для сдачи объекта [L]

- [ ] АОСК — Акты освидетельствования **ответственных конструкций** (по аналогии с АОСР, для несущих)
- [ ] ИГД — Исполнительная геодезическая документация (схемы расположения)
- [ ] КС-11 — Акт приёмки законченного строительством объекта
- [ ] КС-14 — для государственной приёмочной комиссии (если госзаказ)
- [ ] Пакет для Госстройнадзора — автоформирование комплекта документов одной кнопкой
...
- [ ] Email / SMS / Telegram уведомления (расширение внутренних плашек)
- [ ] **Цифровая подпись через SMS-код** — для всех ролей-подписантов (заказчик, технадзор)
