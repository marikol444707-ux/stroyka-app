...
### 🟢 Ф6.3 — Что осталось от Ф6.2 [S]

- [ ] Загрузка tb_journal через GET /tb-journal в loadAll (сейчас саветается локально + дублируется в БД)
- [ ] UI для просмотра истории версий документа (бэк: GET /document-versions нужен)
- [ ] Журнал авторского надзора (если есть проектная организация — отдельная сущность)
...
- [ ] UI для редактирования условий гарантии в карточке проекта (даты начала/окончания, контакт)
