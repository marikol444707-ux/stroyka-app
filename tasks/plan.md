...
- [ ] Подтверждение фактического получения email пользователем (SMTP передал письмо).
...
- [ ] После завершения цепочки — закрепление заказчиков за менеджерами поставщика; P9 пока назначает отдельные КП.
Подробные доказательства и ограничения: docs/supply-live-acceptance.md.

- [ ] Отдельный технический долг общей загрузки: supplier `/online` 403 и фоновые GET 429 при массовом refreshData. Не ослаблять авторизацию/лимиты; перейти к адресному обновлению данных.
...
- [ ] Выбрать: один ответственный или несколько менеджеров на заказчика.
- [ ] Миграция, политика доступа по заказчику и история переназначений.
- [ ] Серверные операции руководителя, конкуренция и отзыв доступа.
- [ ] Приглашение менеджера в существующую компанию поставщика.
- [ ] «Команда» и назначение в «Заказчиках» в том же кабинете.
- [ ] PostgreSQL/HTTP, frontend, реальные браузерные проверки; выпуск и закрытие теста.
