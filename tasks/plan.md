...
- [ ] Task E4: Build a reviewed `old row -> new row` transfer that moves only uncompleted assignment and open-request balances; never move confirmed JPR, signed acts, warehouse history or payments. Draft contract: `docs/estimate-row-transfer-contract.md`.
  ...
  - [ ] Task E4.2: Add an inert reviewed-mapping ledger and draft/approve API with exact owners, source/target snapshots, explicit quantities and deterministic plan hash; do not move balances yet. Local implementation and disposable-PostgreSQL verification are complete on `feature/estimate-row-transfer-ledger`; guarded production schema review/apply, deploy and protected API smoke remain pending.
