...
- [ ] Task E5: Make active-estimate material control use stored `company_id + project_id` throughout instead of project name, then add cross-company collision tests. Accepted contract: `docs/active-estimate-material-control-ownership.md`.
  ...
  - [ ] Task E5.4: Version and validate material-control supply lineage with exact owner IDs, then replace backend name-only active-estimate selection and refresh queries with parameterized company/project predicates. Local implementation `2be1e1bf` + `c9bd2c92` is complete and fully verified; production deployment/audit evidence remains.
