...
- [ ] Task A7: After Tasks E3-E6, run deterministic estimate-revision impact analysis in shadow mode across assignments, materials, supply, warehouse and project economics; do not expose recommendations or mutate business records.
  ...
  - [ ] Task A7.4: Project the exact E6 budget delta/eligibility and compose one versioned, deterministic, bounded five-domain result with counts, IDs and fixed reason codes only. Runtime `3bda0fc50107` deployed and passed public smoke, but the exact production economics audit failed closed on an invalid `estimates.total` schema requirement. Corrective commit `7a3ffb32` now uses recomputed `sections_json` totals plus stored reconciliation totals and includes one guarded E6 function replacement; production redeploy, guarded migration and exact combined audit remain pending.
