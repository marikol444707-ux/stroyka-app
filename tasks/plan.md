...
- [ ] Task A6.1: Add a pure fail-closed change-event contract and deterministic dispatch plan for the first allowlisted `estimate.version_activated -> director.daily_brief` path. Local implementation and verification pass; it performs no SQL, queue/model/network call or business mutation and remains disconnected from runtime until a separate deploy and shadow-wiring step.
- [ ] Task A6.2: Wire the activation contract in metadata-only shadow mode after the three committed estimate activation paths. Local verification passes; responses/logs expose only bounded `planned/rejected` metadata with zero enqueue/business writes, while repeated active state, deactivation and drafts remain ignored. Production deploy and one manual shadow activation are pending.
