...
- [ ] Task E4: Build a reviewed `old row -> new row` transfer that moves only uncompleted assignment and open-request balances; never move confirmed JPR, signed acts, warehouse history or payments. Draft contract: `docs/estimate-row-transfer-contract.md`.
  ...
  - [ ] Task E4.3: Apply approved assignment balances transactionally, preserving confirmed JPR links and negotiated brigade price while recording immutable before/after evidence.
    - [ ] Task E4.3.1: Extend the separately guarded schema with an immutable assignment-transfer receipt, exact entry/plan owner linkage and database mutation guards; keep deploy/init free of automatic DDL.
    - [ ] Task E4.3.2: Add a leadership-only exact-hash assignment-apply endpoint that locks rows in deterministic order, recomputes confirmed JPR, rejects drift/conflicts, splits quantity without changing brigade value and returns an idempotent bounded receipt.
    - [ ] Task E4.3.3: Prove rollback, concurrency, repeated apply and zero writes to JPR/acts/payments/supply/warehouse tables in real PostgreSQL before any production schema or balance action.
