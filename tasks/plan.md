...
- [ ] Task A6.3.2: Design a post-commit activation hand-off behind a disabled-by-default flag. It must preserve the completed estimate action on dispatch failure, enqueue only the exact validated source revision, expose bounded audit metadata, and leave runner execution disabled until a separate decision.
