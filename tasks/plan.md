...
- [ ] Task M7i2: Build a guarded dry-run cleanup plan for the exact `25` orphan child rows while preserving terminal legacy MAX outbox `30/32/34/36/38`. Local guarded dry-run/apply implementation and tests are complete; production dry-run pending, apply remains a separate explicit decision protected by count and SHA.
