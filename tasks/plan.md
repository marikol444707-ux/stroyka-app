...
- [ ] Task A4.2.4: Install and verify the prepared one-company daily one-shot schedule. The local adapter resolves the business date in `Europe/Moscow`, validates the controlled-cycle identity/output and is dry-run by default; the disabled `systemd` templates target company `1` at `07:10 Europe/Moscow` and never start the generic daemon. Focused daily-brief tests pass (`51`), full backend passes (`1172`), frontend passes (`299`) and the production build succeeds. Production systemd verification, manual one-shot, unit installation and timer enablement still require a separate approved rollout.
