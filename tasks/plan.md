...
- [ ] Task A8: Add preview-only supply recommendations and RFQ drafts with exact source lineage.
  ...
  - [x] ~~Task A8.3: In the same rolled-back read-only snapshot, rebuild A8.2 and derive only bounded ID-only company-link/account-ready supplier candidates for human review; material qualification, ranking, selection and sending remain forbidden.~~ Local A8 `24/24`, A7 `117/117`, full backend `1744/1744`, compilation/static checks and independent reviews pass. The canonical schema still lacks the required `suppliers(user_id,id)` read index, so a nonempty candidate path remains explicitly `incomplete` until A8.3a; no route/UI/job/schema write or deployment was added.
  - [ ] Task A8.3a: Add and verify the separately approved canonical `suppliers(user_id,id)` read index before enabling a nonempty A8.3 supplier-review path.
