...
- [ ] Task M6.6d1: Add guarded `ai_tasks` ownership with explicit `company` or `platform` scope without changing task runtime. Implemented locally; production dry-run/apply pending.
- [ ] Task M6.6d2: Scope direct `ai_tasks` list/create/update and assignment actions through a verified finding/project owner while keeping `Система` in platform-only scope.
