...
- [ ] Task A5.1: Deploy and verify the locally complete read-only attention projection. It exposes only critical/warning items from the latest validated single-company brief, caps visible rows at 12, uses immutable server-owned reason/action policy, and renders no action buttons. Local focused/full tests, build and desktop/mobile Chrome review pass; production and protected verification remain.
