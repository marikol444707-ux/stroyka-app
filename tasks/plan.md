...
- [ ] Task M6.6b3: Remove the legacy global primary key on `project_name` only after M6.6b2 is live, preserving unique `(company_id,project_id)` so different companies may use the same project name. Guarded cutover implemented locally; production dry-run/apply pending.
