...
- [ ] Task E4: Build a reviewed `old row -> new row` transfer that moves only uncompleted assignment and open-request balances; never move confirmed JPR, signed acts, warehouse history or payments. Draft contract: `docs/estimate-row-transfer-contract.md`.
  ...
  - [ ] Task E4.3: Apply approved assignment balances transactionally, preserving confirmed JPR links and negotiated brigade price while recording immutable before/after evidence. Implemented and fully verified locally; guarded production schema review, deploy and an owner-approved real-plan apply remain pending.
