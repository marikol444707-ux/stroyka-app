...
- [ ] Task A7: After Tasks E3-E6, run deterministic estimate-revision impact analysis in shadow mode across assignments, materials, supply, warehouse and project economics; do not expose recommendations or mutate business records.
  ...
  - [ ] Task A7.4: Project the exact E6 budget delta/eligibility and compose one versioned, deterministic, bounded five-domain result with counts, IDs and fixed reason codes only. Complete locally in `b0f33e58` + `8bc2d698`; production deploy and exact combined audit pending.
