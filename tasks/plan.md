...
- [ ] Task M6.4j: Scope direct `PUT/DELETE /unexpected-works/{id}` by the selected-company actor and exact stored owner; carry `company_id` into the approval-created work journal. Implemented locally; release pending.
- [ ] Task M6.4k: Scope `/estimates/{id}/include-changes` and `/estimates/{id}/reconcile-changes` through one verified estimate/project owner and update only same-owner change IDs.
