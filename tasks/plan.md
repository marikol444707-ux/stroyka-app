...
- [ ] Task E3: Add immutable source estimate/version ownership to brigade assignment rows and prevent a new revision from overwriting historical quantities or manual brigade prices.
  ...
  - [ ] Task E3.4: Add FK/CHECK/index/immutability and delete-restriction gates, then prove writer and constraint readiness before enforcement.
    ...
    - [ ] Task E3.4.2: Fix exact estimate deletion blocking and apply the separately guarded, idempotent strict-lineage schema only after E3.4.1 production evidence is reviewed.
      - [ ] Task E3.4.2a: Follow stored `source_estimate_version_id` exactly during estimate deletion and scope compatibility-key matching to explicit `legacy` rows; deploy and verify the read-only production audit before any DDL.
      - [ ] Task E3.4.2b: Apply the separately reviewed guarded FK/CHECK/index/trigger/default migration only after E3.4.2a is production-green.
