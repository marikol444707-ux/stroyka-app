...
- [x] ~~Task A4.2.2: Add a local fail-closed exact-job runner hand-off. `--once --job-id <id>` atomically claims only that queued, due, attempt-eligible ID from the immutable handler allowlist, skips global recovery and never falls through to a neighboring row. Unclaimable targets return metadata-only `not_claimed` with exit code `2`; `--job-id` without `--once` is rejected. No schedule, daemon, model, MAX or business mutation is enabled.~~
- [x] ~~Task A4.2.3: Add a local dry-run-by-default controlled cycle for one explicit company/date. Explicit apply commits the idempotent producer first and then executes only its returned exact job ID after validating company/date/type. Existing success is not rerun; every other nonqueued or unclaimable state fails closed. Business writes remain zero and no schedule, daemon, model, MAX or fan-out is enabled.~~
