...
- [ ] Task E6: Add an explicit approved budget-adjustment event so a new estimate total can update project economics without rewriting accounting history. The business contract in `docs/approved-budget-adjustment-event.md` was accepted on 2026-08-07; implementation remains split into separately reviewed, reversible slices.
  ...
  - [x] Task E6.5: Add an explicit preview/confirm/history UI for authorized leaders, with no automatic apply during reconciliation approval or estimate activation. The local UI validates exact canonical evidence, submits only one preview hash, fails closed on stale/conflict responses, uses the selected-company role, lazily loads immutable history and passed `31/31` focused tests plus desktop/mobile Chromium verification with zero console errors.
  - [ ] Task E6.6: Run final focused/full tests, real-PostgreSQL concurrency and protected-history proofs, build, deploy smoke and read-only production cutover audit; never manufacture a production reconciliation. Local gates are green: E6 package `117` tests (`7` fixture skips covered by the separate PostgreSQL run), backend `1598` tests (`21` expected skips), frontend `342/342`, dedicated PostgreSQL `7/7`, production build and browser verification; the final production deploy/smoke/read-only audit remains.
