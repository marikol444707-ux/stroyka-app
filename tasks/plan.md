...
- [ ] Task A6.3.2: Deploy the locally completed post-commit activation handoff with both controls absent (`AGENT_CHANGE_DISPATCH_APPLY=true` plus strict `AGENT_CHANGE_DISPATCH_COMPANY_IDS` are both required). Local focused `30/30`, backend `1209/1209`, frontend `299/299` and production build pass. Dispatch uses a separate short queue transaction, rolls it back with bounded `dispatch_unavailable` on failure, and cannot undo the completed estimate action; runner/model/business writes remain disabled. Production deploy in shadow mode and one repeat activation check are pending.
