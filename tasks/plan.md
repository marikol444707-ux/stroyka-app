...
- [ ] Task E5: Make active-estimate material control use stored `company_id + project_id` throughout instead of project name, then add cross-company collision tests. Accepted contract: `docs/active-estimate-material-control-ownership.md`.
  - [ ] Task E5.1: Add a bounded, rolled-back readiness audit for project/active-estimate owner tuples and the static name-only selector/query inventory; do not change schema or runtime behavior. Local implementation and release gates pass; production deployment/audit evidence is pending.
    ...
    - [ ] Task E5.1.2: Deploy the inert audit command and capture production `dataReady`, six expected name-scoped boundaries, zero writes and rollback evidence.
